...
class TestRepo {
  ...
  Future<RemoteBaseModel<AppSettings>> getLandingData({
    ...
    JDRepoConsole.info("Getting landing data in repo",
    ...
    if (result.error != null) {
      JDRepoConsole.error("Source error in getLandingData: ${result.error?.message}",
          context: LogContext(module: "TestRepo", method: "getLandingData"));
    ...
    if (result.data != null) {
      try {
        ...
        JDRepoConsole.success("Landing data parsed successfully in repo",
            context: LogContext(module: "TestRepo", method: "getLandingData"));
        return RemoteBaseModel(data: settings, status: StatusModel.success);
      } catch (e) {
        JDRepoConsole.error("Parsing error in getLandingData: $e",
            context: LogContext(module: "TestRepo", method: "getLandingData", metadata: result.data));
        return RemoteBaseModel(
          status: StatusModel.error,
    ...
    JDRepoConsole.warn("No data found in getLandingData",
        context: LogContext(module: "TestRepo", method: "getLandingData"));
